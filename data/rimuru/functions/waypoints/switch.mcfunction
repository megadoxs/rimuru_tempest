execute if score @s rimuru_active_switch matches 1100 store result score @s rimuru_active_switch run data get entity @s cardinal_components."apoli:powers".Powers[{Type:"rimuru:switch_active_switch"}].Data
...
execute if score @s rimuru_active_switch matches 1100 if score @s rimuru_waypoints_switch matches 1..2 run resource change @s rimuru:waypoints_bar 1
