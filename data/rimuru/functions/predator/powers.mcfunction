...
#deactivate lower level ability currently only flight
...
execute if score @s rimuru_predator_fail matches 1 if score @s[nbt={cardinal_components:{"apoli:powers":{Powers:[{Type:"rimuru:commons_skill_bk",Data:1}]}}}] rimuru_predator_random matches 2 run scoreboard players operation @s rimuru_skill_ultrasonic_waves = @s rimuru_switch_temp
